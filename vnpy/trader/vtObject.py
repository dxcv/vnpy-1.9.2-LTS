...
from vnpy.trader.vtConstant import (
    EMPTY_FLOAT, EMPTY_INT, EMPTY_STRING, EMPTY_UNICODE)
...
class VtTickData(VtBaseData):
    ...
    # ----------------------------------------------------------------------
    def __str__(self):
        return 'TICK:%s\tlastPrice:%s\tvolume:%s\tbidPrice1:%s\tbidVolume1:%s\taskPrice1:%s\taskVolume1:%s' % (self.datetime,
                                                                                                               self.lastPrice,
                                                                                                               self.volume,
                                                                                                               self.bidPrice1,
                                                                                                               self.bidVolume1,
...
class VtBarData(VtBaseData):
    ...
    # ----------------------------------------------------------------------
    def __str__(self):
        return 'BAR:%s\topen:%s\thigh:%s\tlow:%s\tclose:%s\tvolume:%s' % (self.datetime,
                                                                          self.open,
                                                                          self.high,
                                                                          self.low,
                                                                          self.close,
...
class VtTradeData(VtBaseData):
    ...
    def __unicode__(self):
        return 'TRADE:%s\torderID:%s\ttradeID:%s\tdirection:%s\toffset:%s\tprice:%s\tvolume:%s' % (self.tradeTime,
                                                                                                   self.orderID,
                                                                                                   self.tradeID,
                                                                                                   self.direction,
                                                                                                   self.offset,
...
class VtOrderData(VtBaseData):
    ...
    def __unicode__(self):
        return 'ORDER:%s\torderID:%s\tdirection:%s\toffset:%s\tprice:%s\ttotalVolume:%s\tstatus:%s' % (self.orderTime,
                                                                                                       self.orderID,
                                                                                                       self.direction,
                                                                                                       self.offset,
                                                                                                       self.price,
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if cls not in cls._instances:
            cls._instances[cls] = super(
                VtSingleton, cls).__call__(*args, **kwargs)
